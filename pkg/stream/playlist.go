...
// broadcastPlaylistUpdate wakes all goroutines waiting for a playlist change
// by closing the current broadcast channel and replacing it with a fresh one.
func (s *Stream) broadcastPlaylistUpdate() {
	s.playlistBroadcastMu.Lock()
	old := s.playlistBroadcast
	s.playlistBroadcast = make(chan struct{})
...
//
// Returns (playlist, nextEligibleMs, lastMSN) where nextEligibleMs is the
// timestamp of the first segment not yet eligible (0 if no such segment
// exists) and lastMSN is the highest Media Sequence Number (segment Index)
// in the rendered window (-1 if no segments are eligible).
// Returns ("", nextEligibleMs, -1) when no segments are eligible.
//
// Must be called with s.mu.RLock held.
func (s *Stream) renderMediaPlaylist(nowMs int64, windowSize int) (string, int64, int64) {
	...
	holdBack := float64(s.metadata.TargetDurationSecs) * 3
	b.WriteString("#EXT-X-SERVER-CONTROL:CAN-BLOCK-RELOAD=YES,HOLD-BACK=")
	b.Write(strconv.AppendFloat(scratch[:0], holdBack, 'f', 3, 64))
	b.WriteByte('\n')
	...
	result := b.String()
	lastMSN := int64(window[len(window)-1].Index)
	return result, nextEligibleMs, lastMSN
}
...
func (s *Stream) runPlaylistRenderer(windowSize int) {
	...
	defer s.broadcastPlaylistUpdate() // wake waiters on stream deletion
	...
	for {
		...
		s.mu.RLock()
		playlist, nextEligibleMs, lastMSN := s.renderMediaPlaylist(nowMs, windowSize)
		s.mu.RUnlock()

		p := playlist // allocate a distinct string per iteration for the atomic pointer
		s.cachedPlaylist.Store(&p)
		s.lastRenderedMSN.Store(lastMSN)
		s.broadcastPlaylistUpdate()

...
**Stale-playlist edge case.** A segment URI's baked token expires ~10 minutes past that segment's presentation timestamp, regardless of when the playlist was rendered. For live streams this is a non-issue: segments roll out of the sliding window (tens of seconds) long before their tokens expire. If a stream stops producing new segments, any cached playlist whose segments have aged past TTL will serve `200 OK` but its `?vt=` tokens will be rejected at init/segment fetch with `401`. Viewers using their own long-lived viewer token directly on init/segment URIs are unaffected.
